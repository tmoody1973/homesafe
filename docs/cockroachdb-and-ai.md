...
* Cluster-level read and write access via the <InternalLink version="cockroachcloud" path="connect-to-the-cockroachdb-cloud-mcp-server">CockroachDB Cloud MCP server</InternalLink>.
* Cluster-level read and write access for self-hosted deployments via the <InternalLink path="cockroachdb-mcp-server">CockroachDB MCP Server</InternalLink>.
* Operational workflows encoded as <InternalLink path="agent-skills">Agent Skills for CockroachDB</InternalLink>.
* Command-line automation with the <InternalLink version="cockroachcloud" path="ccloud-get-started">`ccloud` CLI</InternalLink>, designed to be compatible with AI agents.
* Access to the Cockroach Labs public documentation via the <InternalLink path="docs-mcp-integration">CockroachDB Docs MCP server</InternalLink>.

The following sections provide an overview of these tools.

### CockroachDB MCP servers

CockroachDB provides Model Context Protocol (MCP) servers that expose a set of tools for inspecting and querying your clusters from your AI tools. These tools let your AI assistants list <InternalLink path="show-databases">databases</InternalLink> and <InternalLink path="show-tables">tables</InternalLink>, describe <InternalLink path="show-schemas">schemas</InternalLink> and <InternalLink path="show-index">indexes</InternalLink>, inspect cluster health and running queries, and run read-only SQL and <InternalLink path="explain">`EXPLAIN`</InternalLink> statements. When explicitly enabled, they can also <InternalLink path="create-database">create databases</InternalLink>, <InternalLink path="create-table">create tables</InternalLink>, and <InternalLink path="insert">insert rows</InternalLink>.

This allows you to manage and modify a CockroachDB cluster using natural language prompts. For example, you could use the following prompt to instruct your AI tool to interface with the cluster:

```
List all of the tables in the demo_db database.
```

An MCP server is available for both deployment types: CockroachDB Cloud clusters use the managed <InternalLink version="cockroachcloud" path="connect-to-the-cockroachdb-cloud-mcp-server">CockroachDB Cloud MCP server</InternalLink>, and self-hosted clusters use the <InternalLink path="cockroachdb-mcp-server">CockroachDB MCP Server</InternalLink>, which you run yourself.

### Agent Skills for CockroachDB

<InternalLink path="agent-skills">Agent Skills for CockroachDB</InternalLink> are small, structured capabilities that encode CockroachDB operational expertise in a machine-executable format. These skills live in the public [cockroachdb-skills](https://github.com/cockroachlabs/cockroachdb-skills) repository and follow the [Agent Skills Specification](https://agentskills.io/specification), with defined inputs, outputs, and safety guardrails.
...
The <InternalLink version="cockroachcloud" path="ccloud-get-started">`ccloud` CLI</InternalLink> is the command-line interface for CockroachDB Cloud. You can use the `ccloud` CLI to create clusters, manage networking (for example, IP allowlists), create SQL users, retrieve connection information, and more.
...
CockroachDB has a <InternalLink path="vector">`VECTOR` data type</InternalLink> for storing fixed-length floating-point embeddings and supports similarity operators such as L2 distance (`<->`), inner product (`<#>`), and cosine distance (`<=>`). You can index vectors using <InternalLink path="create-index#create-vector-indexes">`CREATE VECTOR INDEX`</InternalLink> and combine them with other indexed columns.

You can store vector embeddings, relational data, and <InternalLink path="jsonb">JSON</InternalLink> metadata in the same table and query them together. For example:
...
CockroachDB's <InternalLink path="transactions">transactional model</InternalLink> provides a foundation for storing agent state, execution history, and coordination metadata. <InternalLink path="demo-serializable">Serializable isolation</InternalLink> ensures that state transitions occur correctly even when multiple agents or processes attempt concurrent updates. The database's <InternalLink path="frequently-asked-questions#how-does-cockroachdb-survive-failures">high availability</InternalLink> design allows agents to continue operating during node or region failures without requiring external coordination services.
...
* <InternalLink path="multiregion-overview">Multi-region deployments</InternalLink> place data closer to users and can enforce <InternalLink path="regional-tables">data locality</InternalLink> or residency requirements while maintaining strong consistency.

CockroachDB provides <InternalLink path="demo-serializable">serializable transactions</InternalLink> by default. Vector indexes participate in the same transaction and index maintenance model as other <InternalLink path="indexes">secondary indexes</InternalLink>, so similarity search results remain aligned with the underlying data. This applies to tables storing embeddings, relational records, and agent state.
